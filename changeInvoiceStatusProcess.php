...
if (isset($_GET["id"])) {
    ...
    if ($status_id == 0) {
        ...
        Database::iud("UPDATE `recent` SET  `status` = '1' WHERE `product_id` = '" . $invoice_data["product_id"] . "'");
    } else if ($status_id == 1) {
        Database::iud("UPDATE `invoice` SET  `status` = '2' WHERE `invoice_id` = '" . $invoice_id . "'");
        Database::iud("UPDATE `recent` SET  `status` = '2' WHERE `product_id` = '" . $invoice_data["product_id"] . "'");
    } else if ($status_id == 2) {
        Database::iud("UPDATE `invoice` SET  `status` = '3' WHERE `invoice_id` = '" . $invoice_id . "'");
        Database::iud("UPDATE `recent` SET  `status` = '3' WHERE `product_id` = '" . $invoice_data["product_id"] . "'");
    } else if ($status_id == 3) {
        Database::iud("UPDATE `invoice` SET  `status` = '4' WHERE `invoice_id` = '" . $invoice_id . "'");
        Database::iud("UPDATE `recent` SET  `status` = '4' WHERE `product_id` = '" . $invoice_data["product_id"] . "'");
    }
