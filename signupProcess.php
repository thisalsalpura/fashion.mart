...
if (empty($fname)) {
    ...
    $emailExistsQuery = "SELECT COUNT(*) as count FROM `user` WHERE `email` = '" . $email . "'";
    $emailExistsResult = Database::search($emailExistsQuery);
    $emailExistsData = $emailExistsResult->fetch_assoc();
    if ($emailExistsData['count'] > 0) {
        echo "A user with the same email already exists. Please enter a different email.";
        exit();
    }

    $mobileExistsQuery = "SELECT COUNT(*) as count FROM `user` WHERE `mobile` = '" . $mobile . "'";
    $mobileExistsResult = Database::search($mobileExistsQuery);
    $mobileExistsData = $mobileExistsResult->fetch_assoc();
    if ($mobileExistsData['count'] > 0) {
        echo "A user with the same mobile already exists. Please enter a different mobile.";
        exit();
    }
